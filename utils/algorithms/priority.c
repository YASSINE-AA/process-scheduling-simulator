    ...
    *executed_tasks_size = 0;
    ...
    while (!is_execution_done(executed, executed_size, process_array, process_array_size))
    {
        ...
        if (!is_pr_queue_empty(queue))
        {
            ...
            if (execute.execute_full == true)
            {
                printf("executing %s fully at %d", execute.name, current_time);
                ...
                finish_track_arr = update_finish_tracker(finish_track_arr, &finish_track_arr_size, execute, execute.execution_time, current_time);
                ...
                for (int i = 0; i < process_array_size; i++)
                {
                    if (process_array[i].arrived_at <= current_time && !is_in_old_list(process_array[i], executed, executed_size))
                    {
                        add_to_pr_queue_p(queue, process_array[i], finish_track_arr, finish_track_arr_size, current_time);
                        arrival_offset[arrival_offset_size] = process_array[i];
                        arrival_offset_size++;
                    }
                }
            }
